...
import {Button, Card, Col, Empty, Row, Spin, Table, Typography} from "antd";
...
class Project extends Component {
    ...
    renderDataset(dataset) {
        return (
            <Card key={dataset.dataset_id} title={dataset.name} extra={<>
                ...
                <Table bordered dataSource={this.props.tables.map(t => ({...t, name: t.name || null}))}
            ...
            <div>
                ...
                <Spin spinning={this.props.loadingDatasets}>
                    {(this.props.datasets || []).length > 0
                        ? this.props.datasets.map(d => this.renderDataset(d))
                        : (
