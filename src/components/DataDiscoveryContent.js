...
class DataDiscoveryContent extends Component {
    ...
    render() {
        return <>
            <SitePageHeader title="Datasets" subTitle="Federated, censored dataset search" />
            <Layout>
                <Layout.Content style={{background: "white", padding: "24px"}}>
                    <Suspense fallback={<Card><Skeleton /></Card>}>
                        <Switch>
                            <Route exact path={withBasePath("data/sets/search")}
                                   component={DiscoverySearchContent} />
                            <Route exact path={withBasePath("data/sets/:dataset")}
                                   component={DiscoveryDatasetContent} />
                            <Redirect from={withBasePath("data/sets")}
